...
#     # ✅ MODIFIED: 从 key 中提取所有字符
#     all_chars = set(
#         k.split(b'+')[1].decode("utf-8")
#         for k in all_keys if b'+' in k
#     )
#     print(f"Total unique characters in LMDB: {len(all_chars)}")

#     # ✅ MODIFIED: 随机选择 4 个字符
#     selected_chars = set(random.sample(list(all_chars), 4))
#     print(f"Randomly selected 4 characters: {selected_chars}")

#     # ✅ MODIFIED: 根据这四个字符过滤 keys
#     filtered_keys = [
#         k.decode("utf-8") for k in all_keys 
#         if b'+' in k and k.split(b'+')[1].decode("utf-8") in selected_chars
#     ]
#     print(f"Filtered {len(filtered_keys)} keys matching selected characters.")
