...
def run_full_training(
    ...
    # 3. 创建数据集和加载器
    print("\n📚 创建训练数据集...")
    full_dataset = FullDatasetPairDataset(
        accessor=accessor,
        decoder=decoder,
        cfg=cfg,
    ...
    print(f"📊 数据集划分: 总共{dataset_size}个样本")
    print(f"  🏋️  训练集: {len(train_indices)}个样本 ({len(train_indices)/dataset_size*100:.1f}%)")
    print(f"  🎯 测试集: {len(test_indices)}个样本 ({len(test_indices)/dataset_size*100:.1f}%)")
    ...
    # 5. 训练模型
    print("\n🎯 开始训练...")
    model = train_full_model(model, train_loader, test_loader, device=device, lr=lr, epochs=epochs, task=task)
...
def train_full_model(model, train_loader, test_loader=None, device="cuda", lr=1e-4, epochs=20, task="content"):
    """训练完整模型 - 使用改进的损失函数，集成tqdm和wandb"""
    ...
    # 🔥 创建checkpoint目录
    ckpt_dir = "/scratch/gz2199/Content-Style-Disentangled-Representation-Learning/0/1/0/ckpt"
    ...
    # 🔥 添加epoch级别的tqdm
    epoch_pbar = tqdm(range(1, epochs+1), desc="Training Epochs", position=0)
    ...
    for ep in epoch_pbar:
        ...
        # 🔥 添加batch级别的tqdm
        batch_pbar = tqdm(train_loader, desc=f"Epoch {ep}", leave=False, position=1)
        
        for i, (x1, x2, y) in enumerate(batch_pbar):
            ...
            batch_pbar.set_postfix({
                'loss': f'{loss.item():.4f}',
                'acc': f'{acc.item():.3f}',
                'avg_loss': f'{tot_loss/num_batches:.4f}',
                'avg_acc': f'{tot_acc/num_batches:.3f}'
            })
            ...
            if (i + 1) % 100 == 0:
                ...
                wandb.log({
                    f"{task}_batch_loss": loss.item(),
                    f"{task}_batch_acc": acc.item(),
                    f"{task}_avg_loss": avg_loss,
                    f"{task}_avg_acc": avg_acc,
                    f"{task}_step": ep * len(train_loader) + i
                })
                print(f"  Batch {i+1}/{len(train_loader)}: loss={avg_loss:.4f}, acc={avg_acc:.3f}")
        
        scheduler.step()
        
        # Epoch总结
        train_loss = tot_loss / num_batches
        train_acc = tot_acc / num_batches
        ...
        if test_loader is not None:
            ...
            with torch.no_grad():
                for x1, x2, y in test_loader:
                    ...
                    test_tot_acc += acc.item()
                    test_num_batches += 1
            
            test_loss = test_tot_loss / test_num_batches
            test_acc = test_tot_acc / test_num_batches
            
            print(f"[Epoch {ep}/{epochs}] Train: loss={train_loss:.4f}, acc={train_acc:.3f} | Test: loss={test_loss:.4f}, acc={test_acc:.3f} | lr={lr_current:.2e}")
        else:
            print(f"[Epoch {ep}/{epochs}] Train: loss={train_loss:.4f}, acc={train_acc:.3f} | lr={lr_current:.2e}")
        
        # 🔥 记录epoch级别的metrics到wandb
        wandb_data = {
            f"{task}_epoch_train_loss": train_loss,
            f"{task}_epoch_train_acc": train_acc,
            f"{task}_learning_rate": lr_current,
            f"{task}_epoch": ep
        }
        if test_loader is not None:
            wandb_data.update({
                f"{task}_epoch_test_loss": test_loss,
                f"{task}_epoch_test_acc": test_acc
            })
        ...
        # 🔥 每个epoch保存checkpoint
        epoch_checkpoint_path = os.path.join(ckpt_dir, f"{task}_epoch_{ep}_{train_timestamp}.pth")
        checkpoint_data = {
            'epoch': ep,
        ...
        print(f"💾 Epoch {ep} checkpoint已保存: {epoch_checkpoint_path}")
    
    # 🔥 关闭epoch进度条
    epoch_pbar.close()
    
    # 🔥 保存最终的checkpoint
    final_checkpoint_path = os.path.join(ckpt_dir, f"{task}_final_{train_timestamp}.pth")
...
def quick_debug_training(encoder_type="enhanced"):
    ...
    # 🔥 初始化wandb
    wandb.init(
        project="font-style-content-disentanglement",
        name=f"debug_training_{encoder_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
        config={
...
def full_scale_training(encoder_type="enhanced"):
    ...
    wandb.init(
        project="font-style-content-disentanglement",
        name=f"full_scale_training_{encoder_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
        config={
            "encoder_type": encoder_type,
