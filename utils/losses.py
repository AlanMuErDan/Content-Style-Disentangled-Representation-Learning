...
class GANLossModule(nn.Module):
    ...
    def forward(self, real_img, fake_img, mode):
        ...
        if mode == 1:
            ...
            # 👇 新增：将 real_img resize 成和 fake_img 一样大小
            if real_img.shape[-1] != fake_img.shape[-1]:
                real_img = torch.nn.functional.interpolate(real_img, size=fake_img.shape[-2:], mode='bilinear', align_corners=False)
