...
from utils.losses import GANLossModule
...
def train_vae_loop(config):
    ...
    print("👉Decoder expected img_size:", config["img_size"])
    ...
    gen_scheduler = LambdaLR(gen_optim, lr_lambda=lr_lambda, last_epoch=-1)
    if use_gan:
        disc_scheduler = LambdaLR(disc_optim, lr_lambda=lr_lambda, last_epoch=-1)
